...
home_folder='/users/jvergara/BIAS_PRECIP_ML/'
...
from create_model_with_eager import *
...
new_map=input_tensor_val[:,:46718]+predictions
new_map[new_map<0]=0

#%%
corrected_bias_val=new_map-obs_data_val
corrected_ae_val=np.absolute((new_map-obs_data_val))
plt.figure(figsize=(15,15))
#jle.Quick_plot,,metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day')
plt.subplot(221)
jle.Quick_plot(reconstruct_array((input_tensor_val[:,:46718]-obs_data_val).mean(axis=0),switzerland_mask),'Real Bias validation RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(222)
jle.Quick_plot(reconstruct_array((np.absolute(input_tensor_val[:,:46718]-obs_data_val)).mean(axis=0),switzerland_mask),'Real MAE validation RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(223)
jle.Quick_plot(reconstruct_array((corrected_bias_val).mean(axis=0),switzerland_mask),
               'Corrected Bias validation using ML RdisaggH-COSMOpompa MB=%1.2f'%np.nanmean(corrected_bias_val)+' MAB=%1.2f'%np.nanmean(np.abs(corrected_bias_val.mean(axis=0))),
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(224)
jle.Quick_plot(reconstruct_array((corrected_ae_val).mean(axis=0),switzerland_mask),'Corrected MAE validation using ML RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.savefig(home_folder+"ML_validation_grid")
#%%
MAE_change=reconstruct_array((corrected_ae_val).mean(axis=0),switzerland_mask)-reconstruct_array((np.absolute(input_tensor_val[:,:46718]-obs_data_val)).mean(axis=0),switzerland_mask)
...
plt.savefig(home_folder+"ML_MAE_change_validation")
...
predictions=predictions.numpy()
#predictions_2D=reconstruct_array(predictions.mean(axis=0),switzerland_mask)
new_map=input_tensor_train[:,:46718]+predictions
new_map[new_map<0]=0
corrected_bias_train=new_map-obs_data_train
corrected_ae_train=np.absolute((new_map-obs_data_train))
plt.figure(figsize=(15,15))
#jle.Quick_plot,,metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day')
plt.subplot(221)
jle.Quick_plot(reconstruct_array((input_tensor_train[:,:46718]-obs_data_train).mean(axis=0),switzerland_mask),'Real Bias training RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(222)
jle.Quick_plot(reconstruct_array((np.absolute(input_tensor_train[:,:46718]-obs_data_train)).mean(axis=0),switzerland_mask),'Real MAE trainign RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(223)
jle.Quick_plot(reconstruct_array((corrected_bias_train).mean(axis=0),switzerland_mask),
               'Corrected Bias training using ML RdisaggH-COSMOpompa MB=%1.2f'%np.nanmean(corrected_bias_train)+' MAB=%1.2f'%np.nanmean(np.abs(corrected_bias_train.mean(axis=0))),
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)

plt.subplot(224)
jle.Quick_plot(reconstruct_array((corrected_ae_train).mean(axis=0),switzerland_mask),'Corrected MAE training using ML RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.savefig(home_folder+"ML_train_grid")
...
jle.Quick_plot(MAE_change,
               ...
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu_r,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.savefig(home_folder+"ML_MAE_change_train")


#%%
#corrected_map_val_QM=np.load(store_folder+'quantile_mapped_precip_validation_with_apply_along_axis.npy')
#corrected_map_val_QM=np.load(store_folder+'QM_precip.npy')
corrected_map_val_QM=np.load(store_folder+'QM_prec.npy')[1461:,:]
...
jle.Quick_plot(reconstruct_array((np.absolute(input_tensor_val[:,:46718]-obs_data_val)).mean(axis=0),switzerland_mask),'Real MAE validation RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(223)
jle.Quick_plot(bias_QM_val,'Corrected Bias validation using QM RdisaggH-COSMOpompa MB=%1.2f'%np.nanmean(bias_QM_val)+' MAB=%1.2f'%np.nanmean(np.abs(bias_QM_val)) ,
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(224)
jle.Quick_plot(MAE_QM_val,'Corrected MAE validation using QM RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.savefig(home_folder+"QM_validation_grid")
print(bias_QM_val.mean())
#%%
plt.figure(figsize=(8,8))
...
#%%
corrected_map_QM=np.load(store_folder+'QM_prec.npy')
corrected_map_QM_roll=np.roll(corrected_map_QM,0,axis=1)
corrected_map_train_QM=corrected_map_QM_roll[:1461,:]

bias_QM_train=corrected_map_train_QM-obs_data_train

#for i in range(10):
#    plt.plot(corrected_map_train_QM[i],'o')
#%%
#plt.plot(corrected_map_train_QM.mean(axis=0),alpha=0.5,c='red')
#plt.plot(obs_data_train.mean(axis=0),alpha=0.5,c='y')
#plt.plot(input_tensor_train[:,:46718].mean(axis=0),alpha=0.5,c='g')


#plt.plot(bias_QM_train.mean(axis=0))
#%%
bias_QM_train=reconstruct_array(bias_QM_train.mean(axis=0),switzerland_mask)
MAE_QM_train=np.abs(corrected_map_train_QM-obs_data_train)
MAE_QM_train=reconstruct_array(MAE_QM_train.mean(axis=0),switzerland_mask)
#%%
plt.figure(figsize=(15,15))
#jle.Quick_plot,,metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day')
plt.subplot(221)
jle.Quick_plot(reconstruct_array((input_tensor_train[:,:46718]-obs_data_train).mean(axis=0),switzerland_mask),'Real Bias training RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(222)
jle.Quick_plot(reconstruct_array((np.absolute(input_tensor_train[:,:46718]-obs_data_train)).mean(axis=0),switzerland_mask),'Real MAE training RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(223)
jle.Quick_plot(bias_QM_train,'Corrected Bias training using QM RdisaggH-COSMOpompa MB=%1.2f'%np.nanmean(bias_QM_train),
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(224)
jle.Quick_plot(MAE_QM_train,'Corrected MAE training using QM RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.savefig(home_folder+"QM_training_grid")
print(np.nanmean(bias_QM_train))

#%%
plt.figure(figsize=(8,8))
MAE_change_QM=MAE_QM_train-reconstruct_array((np.absolute(input_tensor_train[:,:46718]-obs_data_train)).mean(axis=0),switzerland_mask)
jle.Quick_plot(MAE_change_QM,
               'Change in MAE Corrected-True training using QM %1.3f'%np.nanmean(MAE_change_QM),
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu_r,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.savefig(home_folder+"QM_MAE_change_training")
