...
model_data=np.load(store_folder+'COSMO_daily_cumulated_prec_random.npy')
#model_data=model_data[:1096]
obs_data=np.load(store_folder+'RdisaggH_daily_cumulated_prec_random.npy')[:model_data.shape[0],]
...
plt.subplot(223)
jle.Quick_plot(reconstruct_array((corrected_bias_val).mean(axis=0),switzerland_mask),'Corrected Bias validation using ML RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(224)
...
jle.Quick_plot(reconstruct_array((np.absolute(input_tensor_train[:,:46718]-obs_data_train)).mean(axis=0),switzerland_mask),'Real MAE trainign RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels_mae,extend='max',cmap=cmap_MAE,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(223)
jle.Quick_plot(reconstruct_array((corrected_bias_train).mean(axis=0),switzerland_mask),'Corrected Bias training using ML RdisaggH-COSMOpompa ',
               metadata_dataset=ds_lat_lon,levels=levels,extend='both',cmap=plt.cm.RdBu,cb_format="%1.2f",cb_label='mm/day',new_fig=0)
plt.subplot(224)
...
#%%
corrected_map_val_QM=np.load(store_folder+'quantile_mapped_precip_validation_with_apply_along_axis_randomized.npy')
